...
class TinderLikeCard extends React.Component {
    ...
    render() {
        ...
        const newList = !list.length ? "" : list.map((card, key) => (React.createElement(StyledLi, { bgStatus: this.state.stateOfContent, background: card.val, duration: props.duration, direction: props.direction, width: props.width, height: props.height, key: key, pose: [card.out, card.out2, card.middle, card.in], order: key }, Array.isArray(props.children) ? props.children[key] : props.children))).reverse();
        return (React.createElement(StyledUl, { className: props.className }, newList));
    }
}
