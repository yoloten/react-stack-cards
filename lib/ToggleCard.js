...
class ToggleCard extends React.Component {
    ...
    render() {
        ...
        const newList = list.map((obj, key) => (React.createElement(StyledLi, { bgStatus: this.state.stateOfContent, background: obj, duration: props.duration, direction: props.direction, width: props.width, height: props.height, key: key, order: key, pose: this.state.toggle, className: props.className }, Array.isArray(props.children) ? props.children[key] : props.children))).reverse();
        return (React.createElement(StyledUl, null, newList));
    }
}
